# Brownfield Agent — memberhub

**Run ID:** n977k5whqjte93kzbdajk3hv4n83rkb2
**Project ID:** jd79s7yb27yrzttm1gp846bhwn83pxrt
**Repo:** /Users/damonbodine/Factory512/builds/memberhub-20260326-223059/memberhub
**Mode:** feature
**Request:** Add 4 AI features using OpenRouter API (OPENROUTER_API_KEY env var, model: nvidia/nemotron-3-super-120b-a12b:free). Create convex/ai.ts with a reusable action pattern. Features: 1) Renewal Risk Predictor — score each member's renewal likelihood 1-100 based on engagement history (event attendance, communication responses), payment history, membership duration, tier changes. Flag at-risk members 60 days before renewal. Show on member detail page and dashboard widget. 2) Personalized Renewal Outreach — draft personalized renewal reminders based on engagement profile. For engaged members celebrate involvement. For disengaged acknowledge gap and re-pitch value. 3) Event Recommendation Engine — for each member, analyze interests from past event attendance and demographics, recommend upcoming events with reasoning. Show on member detail page. 4) Membership Trend Narrator — from dashboard data (counts by tier, revenue, retention), generate narrative insights report with trends, concerns, and recommendations. Each feature needs: Convex action, React component, integration into existing pages.
...
5. Work ONLY inside the target repository at /Users/damonbodine/Factory512/builds/memberhub-20260326-223059/memberhub. Never create files outside it.
...
**Query ready steps:**
```
npx convex run --url https://friendly-elk-878.convex.cloud brownfield/queries:getReadySteps '{"runId":"n977k5whqjte93kzbdajk3hv4n83rkb2"}'
```

**Get run status (for resume):**
```
npx convex run --url https://friendly-elk-878.convex.cloud brownfield/queries:getRunStatus '{"runId":"n977k5whqjte93kzbdajk3hv4n83rkb2"}'
```

**Load artifacts by kind:**
```
npx convex run --url https://friendly-elk-878.convex.cloud brownfield/queries:getArtifactsByKinds '{"runId":"n977k5whqjte93kzbdajk3hv4n83rkb2","kinds":["kind1","kind2"]}'
```

**Push artifact (marks step completed):**
```
npx convex run --url https://friendly-elk-878.convex.cloud brownfield/cliComplete:completeStepWithArtifact '{"runId":"n977k5whqjte93kzbdajk3hv4n83rkb2","stepNumber":N,"artifactKind":"kind","artifactContent":"<json-string>","contentType":"json"}'
```

**Complete step with no artifact:**
```
npx convex run --url https://friendly-elk-878.convex.cloud brownfield/cliComplete:completeStepNoArtifact '{"runId":"n977k5whqjte93kzbdajk3hv4n83rkb2","stepNumber":N}'
```
...
### INITIALIZE
1. `cd` to `/Users/damonbodine/Factory512/builds/memberhub-20260326-223059/memberhub`.
...
Parse `Add 4 AI features using OpenRouter API (OPENROUTER_API_KEY env var, model: nvidia/nemotron-3-super-120b-a12b:free). Create convex/ai.ts with a reusable action pattern. Features: 1) Renewal Risk Predictor — score each member's renewal likelihood 1-100 based on engagement history (event attendance, communication responses), payment history, membership duration, tier changes. Flag at-risk members 60 days before renewal. Show on member detail page and dashboard widget. 2) Personalized Renewal Outreach — draft personalized renewal reminders based on engagement profile. For engaged members celebrate involvement. For disengaged acknowledge gap and re-pitch value. 3) Event Recommendation Engine — for each member, analyze interests from past event attendance and demographics, recommend upcoming events with reasoning. Show on member detail page. 4) Membership Trend Narrator — from dashboard data (counts by tier, revenue, retention), generate narrative insights report with trends, concerns, and recommendations. Each feature needs: Convex action, React component, integration into existing pages.` against the `codebase_profile`.
...
```json
{
  "runId": "n977k5whqjte93kzbdajk3hv4n83rkb2",
  "projectName": "memberhub",
  "mode": "feature",
  "request": "Add 4 AI features using OpenRouter API (OPENROUTER_API_KEY env var, model: nvidia/nemotron-3-super-120b-a12b:free). Create convex/ai.ts with a reusable action pattern. Features: 1) Renewal Risk Predictor — score each member's renewal likelihood 1-100 based on engagement history (event attendance, communication responses), payment history, membership duration, tier changes. Flag at-risk members 60 days before renewal. Show on member detail page and dashboard widget. 2) Personalized Renewal Outreach — draft personalized renewal reminders based on engagement profile. For engaged members celebrate involvement. For disengaged acknowledge gap and re-pitch value. 3) Event Recommendation Engine — for each member, analyze interests from past event attendance and demographics, recommend upcoming events with reasoning. Show on member detail page. 4) Membership Trend Narrator — from dashboard data (counts by tier, revenue, retention), generate narrative insights report with trends, concerns, and recommendations. Each feature needs: Convex action, React component, integration into existing pages.",
  "summary": "...",
  "stepsCompleted": 0,
...
1. `cd /Users/damonbodine/Factory512/builds/memberhub-20260326-223059/memberhub`
